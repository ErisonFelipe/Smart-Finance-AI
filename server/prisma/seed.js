...
async function main() {
  ...
  const categories = [
    ...
    { name: "Investimentos", icon: "trending-up", type: "income", isDefault: true },
    // Investimentos
  ...
  createdCategories.forEach((c) => {
    if (!categoryMap[c.type]) categoryMap[c.type] = [];
    categoryMap[c.type].push(c);
  });

  for (const trans of transactions) {
    const catList = categoryMap[trans.type];
  ...
  const expenseCategories = categoryMap["expense"];
  ...
  console.log(`✅ ${debts.length} dívidas criadas com parcelas`);
